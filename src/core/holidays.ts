...
import { isSameDay, addDays } from './date-utils';
 ...
 */
export function getHolidaysForDate(date: Date, options?: HolidayOptions): Holiday[] {
  ...
  return holidays.filter(holiday => {
    ...
    if (options?.country && holiday.country && holiday.country !== options.country) {
      return false;
    }
    
    // If region filter is provided, check if the holiday is for that region
    if (options?.region && holiday.region && holiday.region !== options.region) {
      return false;
    }
 ...
 */
export function getHolidaysForMonth(year: number, month: number, options?: HolidayOptions): Holiday[] {
  const holidays = getAllHolidays();
 ...
 */
export function getUpcomingHolidays(date: Date, count: number = 5, options?: HolidayOptions): Holiday[] {
  const holidays = getAllHolidays();
 ...
 */
export function getNextHoliday(date: Date, options?: HolidayOptions): Holiday | null {
